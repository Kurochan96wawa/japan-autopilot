 ...
 *
 * セキュリティ: littletabi.com からのCORSのみ許可。入力は長さ制限。出力は<script>除去。
 */
...
export default {
  async fetch(request, env) {
    ...
    const prompt =
      `You are a warm, practical family-travel planner for Japan. Build a ${days}-day itinerary for a family ` +
      `visiting ${cities} with children aged ${ages}. Interests: ${interests || "general kid-friendly sightseeing"}. ` +
      `Pace: ${pace}.\n` +
      `Rules: be specific (areas, named attractions, approx durations and walking distances), kid-friendly ` +
      `(plan naps/meal stops, note stroller access), and HONEST — never invent prices or hours; if relevant say ` +
      `"(check the official site)". Keep each day to 3-5 short bullet points plus ONE practical parent tip.\n` +
      `Output CLEAN HTML ONLY: for each day a <h3>Day N — area</h3> then a <ul><li>…</li></ul>, then ` +
      `<p class="tip"><strong>Tip:</strong> …</p>. No markdown, no preamble, no closing summary.`;
