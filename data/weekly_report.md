# 週次PDCAレポート 2026-09-15

- GA4: 利用可 / 集計期間: 直近28日
- 対象記事: 53件 （Winner 0 / Fixable 0 / Loser 0 / データ不足 53）
- Pinterest指標: **取得不可**（API経由の投稿が無く pinterest_pin_id が未記録）。露出・外部クリックが全記事0で固定されるため、Winner/Fixable/Loser の分類は事実上GA4のPVのみに依存し、下の『データ不足』は「新しすぎる」ではなく「判定材料が無い」の意味になる。

## Winners（伸びている → 横展開）
勝ち筋。似たテーマ・キーワードを増やす。boost_keywordsに反映済み。
- （該当なし）

## Fixable（露出はあるが伸び切らない → テコ入れ）
新角度の再Pin/本文補強の対象。needs_refreshフラグを付与。
- （該当なし）

## Losers（低調 → 避ける/作り直し候補）
テーマ自体が弱い可能性。avoid_keywordsに反映済み。
- （該当なし）

## データ不足（新しすぎ・判定保留）
もう1〜2週ようすを見る。
- **Child-Friendly Ramen in Japan (2026)** — PV 26, outbound 0 (CTR 0.0%), imp 0  `child-friendly-ramen-in-japan-broth-types-spots`
- **Kid-Friendly Sushi in Japan (2026): A Parent's Guide** — PV 19, outbound 0 (CTR 0.0%), imp 0  `kid-friendly-sushi-in-japan-a-guide-for-young-palates`
- **Best Family Hotels in Tokyo: Connecting Rooms & Kitchenettes (2026)** — PV 14, outbound 0 (CTR 0.0%), imp 0  `best-family-hotels-tokyo-connecting-rooms`
- **Tokyo Disneyland vs DisneySea with Kids: Which Park? (2026)** — PV 6, outbound 0 (CTR 0.0%), imp 0  `tokyo-disneyland-vs-disneysea-young-kids`
- **Japan Public Transport with Kids: IC Cards & Strollers (2026)** — PV 5, outbound 0 (CTR 0.0%), imp 0  `japan-public-transport-with-kids-ic-cards-strollers-2026`
- **Navigating Food Allergies in Japan with Kids (2026)** — PV 4, outbound 0 (CTR 0.0%), imp 0  `navigating-food-allergies-in-japan-with-kids-a-guide`
- **Getting Around Japan with a Baby Carrier in 2026** — PV 3, outbound 0 (CTR 0.0%), imp 0  `baby-carriers-in-japan-your-essential-guide-for-family-trave`
- **Visiting Japan Temples with Kids Respectfully (2026 Guide)** — PV 3, outbound 0 (CTR 0.0%), imp 0  `visiting-japan-temples-with-kids-respectfully-2026-guide`
- **Navigating Japan's Public Transport with Kids in 2026** — PV 2, outbound 0 (CTR 0.0%), imp 0  `japan-public-transport-with-kids-fares-strollers-facilities`
- **Basic Japanese Phrases for Families (2026)** — PV 2, outbound 0 (CTR 0.0%), imp 0  `japanese-phrases-for-families-essential-travel-guide`
- **Keeping Kids Happy on Long Japan Journeys: Flights & Trains (2026)** — PV 2, outbound 0 (CTR 0.0%), imp 0  `keeping-kids-happy-on-long-japan-journeys-flights-trains-202`
- **Stroller-Friendly Tokyo: Family Travel Tips for 2026** — PV 1, outbound 0 (CTR 0.0%), imp 0  `stroller-friendly-tokyo-navigating-the-city-with-kids`
- **Best Family-Friendly Hotels in Tokyo (2026)** — PV 1, outbound 0 (CTR 0.0%), imp 0  `tokyo-family-hotels-connecting-rooms-kitchenettes`
- **Renting a Car in Japan with Car Seats: Family Tips for 2026** — PV 1, outbound 0 (CTR 0.0%), imp 0  `renting-a-car-in-japan-with-car-seats-family-travel-guide`
- **Exploring Kyoto with a Stroller in 2026** — PV 1, outbound 0 (CTR 0.0%), imp 0  `kyoto-with-a-stroller-accessible-routes-kid-friendly-spots`

## 戦略アップデート
- boost_keywords: []
- avoid_keywords: []

_このレポートは weekly-improve により自動生成・上書きされます。_
## 重複チェック

類似度0.86以上の重複ペアは検出されませんでした。

## サイト健全性（GSC・表示回数の前週比）

- 今週: 表示 17 / クリック 0
- 前週: 表示 28 / クリック 0
- 急落なし。

## SEO改善（GSC連動）
- 集計: 直近90日 / 対象ページ117 / クエリ268

### タイトル/メタを自動改善（低CTR）
- `kid-friendly-sushi-in-japan-a-guide-for-young-palates` → 「Best Sushi for Kids & Families in Japan (2026 Guide)」（imp 379, CTR 0.0%, 順位 7.7）
- `renting-a-car-in-japan-with-car-seats-family-travel-guide` → 「Japan Child Seat Laws for Rental Cars: Under 6 & Fees」（imp 326, CTR 0.3%, 順位 9.6）
- `renting-a-car-in-japan-with-car-seats-family-travel-guide` → 「Car Seats in Japan: Rules, Rentals & Requirements for Families」（imp 310, CTR 0.6%, 順位 20.1）

### あと一歩ページ（8〜20位 → 強化＋再Pin優先）
- `renting-a-car-in-japan-with-car-seats-family-travel-guide`（順位 9.6, imp 326, CTR 0.3%）
- `kid-friendly-sushi-in-japan-a-guide-for-young-palates`（順位 14.5, imp 147, CTR 0.0%）
- `diapers-formula-in-japan-brands-sizes-where-to-buy`（順位 16.8, imp 143, CTR 0.0%）
- `tokyo-disney-vs-disneysea-for-kids`（順位 15.3, imp 100, CTR 0.0%）
- `child-friendly-ramen-in-japan-broth-types-spots`（順位 8.5, imp 91, CTR 1.1%）
- `tokyo-s-best-themed-cafes-for-families-beyond-maid-cafes`（順位 10.1, imp 84, CTR 1.2%）
- `japan-konbini-snacks-for-picky-eaters-kid-approved`（順位 8.6, imp 59, CTR 1.7%）
- `budget-family-meals-in-japan-where-to-eat-with-kids`（順位 9.6, imp 55, CTR 0.0%）
- `universal-studios-japan-with-kids-maximizing-family-fun`（順位 11.2, imp 52, CTR 0.0%）
- `hands-free-travel-in-japan-with-kids-2026`（順位 13.4, imp 47, CTR 0.0%）

### 新規ネタ起案（検索需要はあるが記事が無いクエリ）
- （該当なし）
