"""SEO仕上げ：①重複(cannibalization)統合 ②ハブのピラー加筆。site構築後に冪等で再適用。

regen/_build_hubs で上書きされても、daily実行のたびに本モジュールが後段で自動的に直す。
...
_TRANSPORT_PILLAR = (
    ...
    'children — but a few family-specific things are worth knowing before you go. Below is the '
    'short version; each linked guide goes deeper.</p>'
    '<h3>IC cards (Suica / PASMO) — tap and go</h3>'
    ...
    'need to be reserved in advance — our '
    ...
    'rides, but for longer drives or rural areas a rental car with properly fitted seats is the safer plan — '
...
def _enrich_hubs() -> int:
    ...
    for slug, block in _HUB_PILLARS.items():
        ...
        if 'id="pillar-transport"' in html:  # 冪等：既に入っていれば何もしない
            continue
        # ハブのカード一覧（<div class="grid">）の直前にピラー本文を差し込む
...
def run() -> dict:
    ...
    h = _enrich_hubs()
    log.info("seo_fixups完了: canonical統合=%d, ハブ加筆=%d", c, h)
    return {"canonicals": c, "hubs": h}
